...
struct cache_element* find(char* url);   //finds the element in cache
int add_cache_element(char* data, char* size, int url);  //adds the element to cache
void remove_cache_element();    //removes the element from cache

int port_number=8080;   //port number for the server
int proxy_socketId;     //socket id for the server
pthread_t tid[MAX_CLIENTS];   //thread id for the clients
sem_t seamaphore;   //seamaphore for the threads
pthread_mutex_t lock;   //mutex for the threads

struct cache_element* cache_head;   
int cache_size;   //cache_size stores the number of elements in cache
...
int main(int argc, char * argv[])
{
    ...
    int client_length;   //length of the client
    struct sockaddr_in server_addr,client_addr;   //server and client address
    
    sem_init(&seamaphore,0,MAX_CLIENTS);   //initializing the seamaphore
    //1. Collecting port number from command line arguments
    if(argv==2){
	...
	bzero((char*)&server_addr, sizeof(server_addr));  
	server_addr.sin_family = AF_INET;   //setting the server address
...
int add_cache_element(char* data,int size,char* url){
    ...
    {   while(cache_size+element_size>MAX_SIZE){
        ...
        struct cache_element* element = (struct cache_element*) malloc(sizeof(cache_element)); // Allocating memory for the new cache element
